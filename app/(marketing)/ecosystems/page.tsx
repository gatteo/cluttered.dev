      ...
      <Container>
        ...
        {ecosystems.length === 0 ? (
          <FadeIn>
            <Card className="text-center py-12">
              <p className="text-text-secondary">Ecosystem documentation coming soon!</p>
            </Card>
          </FadeIn>
        ) : (
                ...
                <Link href={ecosystem.url} className="block group h-full">
                  <Card className="p-6 h-full flex flex-col hover:border-accent-purple/40 transition-colors">
                    <div className="flex items-center gap-3 mb-4">
                      <Image src={getEcosystemIcon(ecosystem.slug)} alt={ecosystem.title} width={40} height={40} className="w-10 h-10" />
                      <h2 className="text-xl font-semibold text-text-primary group-hover:text-accent-purple transition-colors">{ecosystem.title}</h2>
                    </div>

                    <p className="text-text-secondary text-sm mb-4 flex-1">{ecosystem.description}</p>

                    <div className="space-y-3 mb-4">
                      <div className="flex items-center gap-2 text-sm">
                        <HardDrive className="w-4 h-4 text-text-muted" />
                        <span className="text-text-muted">Avg. savings:</span>
                        <span className="text-accent-green font-medium">{ecosystem.averageSize}</span>
                      </div>
        ...
        <FadeIn delay={0.3}>
          <Card className="mt-12 p-8">
            <h2 className="text-2xl font-semibold text-text-primary mb-6 text-center">All Supported Ecosystems</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {ecosystemData.map((eco) => (
                <div key={eco.name} className="flex items-center gap-2 p-3 rounded-lg bg-white/5">
                  <Image src={eco.icon} alt={eco.name} width={24} height={24} className="w-6 h-6" />
